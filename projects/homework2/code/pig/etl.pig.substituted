...
deadevents = FOREACH deadevents
             GENERATE events::patientid as patientid,
                      ...
                      mortality::mtimestamp as mtime; -- detect the events of dead patients and create it of the form (patientid, eventid, value, label, time_difference) where time_difference is the days between index date and each event timestamp*/
/*DUMP deadevents;*/
...
aliveevents = FILTER eventswithmort BY mortality::label is null;-- detect the events of alive patients and create it of the form (patientid, eventid, value, label, time_difference) where time_difference is the days between index date and each event timestamp*/
...
/*-- ****************************************************************************/
filtered = UNION aliveevents, deadevents;-- contains only events for all patients within the observation window of 2000 days and is of the form (patientid, eventid, value, label, time_difference)
...
featureswithid = FOREACH filteredgrpd GENERATE FLATTEN(group) AS (patientid, eventid),
                 COUNT(filtered.eventid) as featurevalue;-- for group of (patientid, eventid), count the number of  events occurred for the patient and create relation of the form (patientid, eventid, featurevalue)
...
/*-- ****************************************************************************/
all_features = FOREACH featureswithid GENERATE eventid; -- compute the set of distinct eventids obtained from previous step, sort them by eventid and then rank these features by eventid to create (idx, eventid). Rank should start from 0.
all_features = DISTINCT all_features;
all_features = RANK all_features by eventid ASC;
all_features = FOREACH all_features GENERATE rank_all_features AS idx,  eventid; -- ($0-1), $1 AS idx, eventid;
...
features = FOREACH features GENERATE featureswithid::patientid as patientid,
                                     ...
                                     featureswithid::featurevalue AS featurevalue; -- perform join of featureswithid and all_features by eventid and replace eventid with idx. It is of the form (patientid, idx, featurevalue)
...
/*-- ****************************************************************************/
maxgroups = GROUP features BY idx;
maxvalues = FOREACH maxgroups GENERATE FLATTEN(group) AS idx, (float)MAX(features.featurevalue) AS maxvalues; -- group events by idx and compute the maximum feature value in each group. I t is of the form (idx, maxvalues)
...
features = FOREACH normalized GENERATE features::patientid AS patientid,
                                       features::idx as idx,
                                       (float)features::featurevalue/maxvalues::maxvalues AS normalizedfeaturevalue; -- compute the final set of normalized features of the form (patientid, idx, normalizedfeaturevalue)*/
...
labels = FOREACH filtered GENERATE patientid, label;-- create it of the form (patientid, label) for dead and alive patients
