...
-- ***** your code below *****
FROM events
RIGHT JOIN mortality m
ON (events.patient_id = m.patient_id);
...
-- ***** your code below *****
FROM (SELECT COUNT(DISTINCT(time)) AS encounter_count FROM alive_events GROUP BY
patient_id) t2;
...
SELECT avg(encounter_count), min(encounter_count), max(encounter_count)
-- ***** your code below *****
FROM (SELECT COUNT(DISTINCT(time)) AS encounter_count FROM dead_events GROUP BY
patient_id) t2;
